...
def main(input, output, robot_ip, frequency, max_duration, num_episodes):
    ...
    assert action_dim == 16, f"Expected 16D bimanual actions, got {action_dim}D"
    ...
    with SharedMemoryManager() as shm_manager:
        ...
        with RealEnv(
            ...
            with torch.no_grad():
                ...
                assert action.shape[-1] == 16, f"Expected 16D actions, got {action.shape[-1]}D"
                del result
            ...
            while episodes_collected < num_episodes:
                ...
                try:
                    ...
                    while True:
                        ...
                        # -------- Prepare Actions --------
                        assert actions.shape[-1] == 16, f"Wrong action dim: {actions.shape[-1]}"
                        ...
                        # actions[:, 0:2] = np.clip(actions[:, 0:2], [-0.5, -0.5], [0.5, 0.5])  # arm1
                        # actions[:, 8:10] = np.clip(actions[:, 8:10], [-0.5, -0.5], [0.5, 0.5])  # arm2


                        actions_robot1 = actions[:, 0:8]  # [pos1(3), quat1(4), gripper1(1)]

                        # Pad with zeros for robot 2 (or use home position)
                        robot2_home = np.array([
                            -0.0350776, 0.368761, 0.0298257,
                            0.382063, -0.055829, 0.922089, 0.0257496,
                            0.0
                        ])

                        # Broadcast robot2_home to match action length
                        actions_full = np.concatenate([
                            actions_robot1, 
                            np.tile(robot2_home, (len(actions), 1))
                        ], axis=1)
                        ...
                        print(f"  Arm1 gripper: {actions[0, 7]}")
                        print(f"  Arm2 pos: {actions[0, 8:11]}")
                        print(f"  Arm2 quat: {actions[0, 11:15]}")
                        print(f"  Arm2 gripper: {actions[0, 15]}")
